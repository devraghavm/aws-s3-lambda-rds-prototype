...
export class MyuiEmployerService implements IService<MyuiRow> {
  constructor() {}
  ...
  // Create operation
  async insert(row: MyuiRow): Promise<any> {
    ...
    try {
      ...
      request.input("run_id", sql.Int, row.run_id ?? null);
      const query = `
  ...
  async insertMany(rows: MyuiRow[]): Promise<any> {
    const pool = await createConnectionPool();
    let transaction = pool.transaction();
    try {
  ...
  // Read operation
  async readAll(): Promise<IResult<MyuiRow[]>> {
    const pool = await createConnectionPool();
    try {
      logger.info("Reading data");
      const request = pool.request();
      let result: IResult<MyuiRow[]> = await request.query(`
        SELECT 
          byte_count,
          tc150_state_code,
          ein,
  ...
  // Read operation
  async readByRunId(run_id: number): Promise<IResult<MyuiRow[]>> {
    const pool = await createConnectionPool();
    try {
      logger.info("Reading data");
      const request = pool.request();
  ...
  // Update operation
  async update(id: number, row: MyuiRow): Promise<any> {
    throw new Error("Method not implemented.");
  }
